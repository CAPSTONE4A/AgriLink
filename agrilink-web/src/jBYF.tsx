...
const registerSchema = z
  .object({
    ...
    // Farmer-only fields (kept optional so buyer can validate)
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-slate-50 px-4 py-10">
      <Card className="max-w-2xl w-full space-y-6">
        ...
        {/* Role tabs */}
        {/* Role tabs with smooth color/section animation */}
        <div className="rounded-2xl border border-slate-200 bg-white p-1">
          ...
          <div className="mt-3 rounded-2xl border border-slate-200 overflow-hidden">
            ...
            <div
              ...
              style={{ transform: `translateX(${watch('role') === 'farmer' ? '0%' : '-100%'})` }}
            >
              {/* Farmer panel */}
              <div className="w-full shrink-0 bg-green-50 p-4 transition-colors duration-500 ease-in-out">
                <form className="grid gap-4 sm:grid-cols-2" onSubmit={handleSubmit(onSubmit)}>
                  ...
                  {/* Farmer-only: avatar url (optional) */}
                  {watch('role') === 'farmer' && (
                    <div className="sm:col-span-2">
                      <Input label="Profile image URL (optional)" {...register('avatarUrl')} />
                    </div>
                  )}
