...
const RegisterPage = () => {
  ...
  const onSubmit = async (values: RegisterFormValues) => {
    try {
      ...
      // role-specific optional fields (only send what matches the selected tab)
      if (values.role === 'farmer') {
        if (values.birthdate) payload.birthdate = values.birthdate;
      }

      if (values.role === 'buyer') {
        if (values.deliveryAddress) payload.deliveryAddress = values.deliveryAddress;
      }
      ...
      // Try multiple common shapes so we display the real backend reason
      const backendMsg =
