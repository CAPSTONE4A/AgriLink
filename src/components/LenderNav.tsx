...
const dropdownItems = [
  { path: '/lender/dashboard', label: 'Dashboard', icon: Crown },
]

export function LenderNav({ loading = false }: { loading?: boolean }) {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white/90 p-3 shadow-sm backdrop-blur-md border-b border-slate-200">
      <div className="mx-auto max-w-7xl flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <p className="text-xs uppercase tracking-[0.28em] text-emerald-700">Lender Portal</p>
          <h1 className="mt-2 text-xl font-semibold text-slate-950">Lender workspace</h1>
        </div>
